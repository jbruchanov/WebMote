...
public class TimePickerWidget extends Composite implements HasValue<Time>,
        HasValueChangeHandlers<Time> {
    private HandlerManager mEventBus = null;
    private static TimePickerWidgetUiBinder uiBinder = GWT
            .create(TimePickerWidgetUiBinder.class);
    ...
    Button btnAddMinute;
    @UiField
    Button btnRemoveHour;
    @UiField
    Button btnRemoveMinute;

    private int mHours = 0;
    private int mMinutes = 0;

    interface TimePickerWidgetUiBinder extends
            UiBinder<Widget, TimePickerWidget> {
    }
    ...
    private void bind() {
        btnAddHour.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                incHour();
            }
        });
        btnAddMinute.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                incMinute();
            }
        });
        btnRemoveHour.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                decHour();
            }
        });
        btnRemoveMinute.addClickHandler(new ClickHandler() {
            @Override
            public void onClick(ClickEvent event) {
                decMinute();
            }
    ...
    protected void setTimeText(int hours, int minutes) {
        String text = StringUtils.get2DecimalValue(hours) + ":"
                + StringUtils.get2DecimalValue(minutes);
