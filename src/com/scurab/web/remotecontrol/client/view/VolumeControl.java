...
public class VolumeControl extends AbstractView implements HasValue<Integer>,
        HasValueChangeHandlers<Integer> {

    private static VolumeControlUiBinder uiBinder = GWT
            .create(VolumeControlUiBinder.class);
    ...
    public VolumeControl() {
        ...
        // mIsPCInternetExplorer = agent.contains("msie") &&
    ...
    private void handleMove(double angle, boolean fireEvent) {
        ...
        double posX = (Math.cos(angle) * lenFromCenter) + getCenterX()
                - leftOffset - imgPointerHalfSize;
        double posY = (Math.sin(angle) * lenFromCenter) + centerY - topOffset
                - imgPointerHalfSize;
    ...
    private void log(int x, int y, double angle) {
        // lblInfo.setText("X:" + x + " Y:" + y + "A:" + Math.round(angle)
    ...
    private int getPercent(double angle) {
        return (int) ((angle - mMin) / 3); // (330 - 30)/100 => (max - min) /
                                           // 100
    }
