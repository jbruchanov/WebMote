...
public class ShutdownPresenter extends BaseControlPresenter {
    ...
    public ShutdownPresenter(DataService dataService, HandlerManager eventBus,
            ShutdownView display) {
    ...
    private void bind() {
        mDisplay.getTimePicker().addValueChangeHandler(
                new ValueChangeHandler<Time>() {
                    @Override
    ...
    protected void onTimeChange(Time time) {
        ...
        mDisplay.getLblOperationStartTime().setText(
                StringUtils.get2DecimalValue(hour) + ":"
    ...
    protected void onSendCommand(String command) {
        if ("Abort".equals(command)
                || Window.confirm(RemoteControl.Words.AreYouSure())) {
