...
{	
	...
	public KeyboardPresenter(DataService dataService, HandlerManager eventBus, KeyboardView display)
	{
	...
	protected void onKeyClick(KeyButton kb)
	{
		...
		else
		{
			...
			if(mShifted)
				kc.KeyCode = R.Constants.KEY_SHIFT;
			kc.KeyCode += getSpecialKeys() + cmd;
			onSendCommand(kc);
		}
	}
	...
	protected void onClickButton(ShowPanel what)
	{
		...
		switch(what)
		{
			case Default: mCurrentKeyboard =   mDisplay.getDefaultKeyboard();break;
			case Shift: mCurrentKeyboard =   mDisplay.getShiftKeyboard();break;
