...
public class DesktopViewPresenter extends BaseControlPresenter {
    ...
    private static final String TEMPLATE = "DesktopView.jpg?w=" + WIDTH_KEY
            + "&h=" + HEIGHT_KEY + "&scale=" + SCALE_KEY + "&compress="
            + COMPRESS_KEY + "&PIN=" + PIN_KEY + "&iter=" + ITER_KEY;

    private static final String TEMPLATE_WEBCLIENT = "desktop/desktop%s.jpg";

    public DesktopViewPresenter(DataService dataService,
            HandlerManager eventBus, DesktopView display) {
    ...
    private void onReloadDesktop() {
        String reqUrl = "";
        if (R.WebClientDemo) {
            reqUrl = TEMPLATE_WEBCLIENT.replace("%s",
                    String.valueOf((mRepeaterCounter++) % 17));
        } else {
            reqUrl = TEMPLATE.replace(WIDTH_KEY,
                    String.valueOf(mDisplay.getImage().getWidth()));
            reqUrl = reqUrl.replace(HEIGHT_KEY,
                    String.valueOf(mDisplay.getImage().getHeight()));
            ...
            reqUrl = reqUrl.replace(ITER_KEY,
                    String.valueOf(mRepeaterCounter++));
        }
