...
public class MediaCenterPresenter extends BaseControlPresenter {
    ...
    public MediaCenterPresenter(DataService dataService,
            HandlerManager eventBus, MediaCenterView display) {
    ...
    public void bind() {
        ...
        mDisplay.getTopPanel().getBtnSpecializedActivity()
                .addClickHandler(new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                        onClickButton(ShowPanel.Spec);
                    }
                });
        mDisplay.getTopPanel().getBtnKeyboard()
                .addClickHandler(new ClickHandler() {
                    @Override
                    public void onClick(ClickEvent event) {
                        onClickButton(ShowPanel.Keyboard);
                    }
                });
    ...
    protected void onClickButton(ShowPanel what) {
        ...
        switch (what) {
            case Default:
                ...
                break;
            case Spec:
                mCurrentVisibleWidget = mDisplay.getSpecializedContainer();
                break;
            case Keyboard: {
                mEventBus.fireEvent(new ChangePresenterEvent(
                        new KeyboardPresenter(mDataService, mEventBus,
                                new KeyboardView())));
                return;
            }
            case User:
    ...
    protected void onSendCommand(Command command) {
        ...
        if (R.MediaCenter.OPEN_AUDIO.equals(command.getMethod())
            ...
            if (open) {
                handleOpeningSpecActivity(translateCommandMethod(command
                        .getMethod()));
            }
        }
    }
        ...
        try {
            mDataService.sendCommand(new InfoCommand(), new RequestCallback() {
                ...
                public void onResponseReceived(Request request,
                        Response response) {
                    try {
                        InfoCommand ic = JsonSimpleParser
                                .parseInfoCommand(response.getText());
                        HashMap<String, List<String>> data = ic
                                .getApplications();
    ...
    public void onOpenSpecializedActivity(String cmd, List<String> apps) {
        ...
        if (RemoteControl.PropertyKeys.AUDIOPLAYER.equals(cmd)) {
            bcp = new AudioPlayerPresenter(mDataService, mEventBus,
                    new AudioPlayerView());
        } else if (RemoteControl.PropertyKeys.VIDEOPLAYER.equals(cmd)) {
            bcp = new VideoPlayerPresenter(mDataService, mEventBus,
                    new VideoPlayerView());
            ...
            bcp = new PicturesPresenter(mDataService, mEventBus,
                    new PicturesView());
        }
    ...
    protected Command getCommand(String command) {
        ApplicationCommand mcc = new ApplicationCommand(
                RemoteControl.MediaCenter, AppType.MediaCenter);
