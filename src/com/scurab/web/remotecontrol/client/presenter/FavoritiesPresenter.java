...
public class FavoritiesPresenter extends BasePresenter
{
	...
	public FavoritiesPresenter(DataService dataService, HandlerManager eventBus, FavoritiesView display)
	{
	...
	private void bind()
	{
		mDisplay.getBtnAdd().addClickHandler(new ClickHandler()
		{
			...
			{	
				onAddItem(getCategory(), mDisplay.getTxtName().getValue(),mDisplay.getTxtLink().getValue());
			}
		});
		...
		mDisplay.getBtnDelete().addClickHandler(new ClickHandler()
		{
			...
			public void onClick(ClickEvent event)
			{
				onDeleteItem(getCategory(), mDisplay.getTxtName().getValue(),mDisplay.getTxtLink().getValue());				
			}
		});
		
		ListBox cats = mDisplay.getCmbType();
		cats.addItem("", "");
		for(String s : new String[] {RemoteControl.PropertyKeys.AUDIOPLAYER, RemoteControl.PropertyKeys.PICTURESVIEWER, RemoteControl.PropertyKeys.VIDEOPLAYER})
		{			
			cats.addItem(s, s);
		}
	...
	{
		return mDisplay.getCmbType().getValue(mDisplay.getCmbType().getSelectedIndex());
	}
	...
	private CommandButton getButton(final String n, final String v)
	{
		...
		b.addClickHandler(new ClickHandler(){@Override public void onClick(ClickEvent event){onButtonClick(n, v);}});
		b.setText(n);
		b.setCommand(v);
		b.setHeight("40px");
		b.setWidth("100%");
	...
	private String getItems()
	{
		...
		while(iter.hasNext())
		{
			...
			sb.append(name + R.Constants.VALUE_SEPARATOR + value + R.Constants.ITEM_SEPARATOR);
		}
		if(sb.length() > 0)
